                ...
                if (
                    ...
                    case hasCategoryAndPriority(request.query) :
                        if (
                            ...
                            data = await database.all(getTodosQuery);
                            response.send(data.map((eachItem)=>outPuttResult(eachItem)));
                        } else {
                            response.status(400);
                            response.send("Invalid Todo Prioriy");
                        }
                }
                else {
                    response.status(400);
                ...
                case hasPriorityProperty(request.query):
                    ...
                    case hasStatusProperty(request.query):
                        ...
                        case hasSearchProperty(request.query):
                            getTodosQuery = `select * from todo where like '%${search_q}%';`;
                            data = await database.all(getTodosQuery);
                            response.send(data.map((eachItem) => outPutResult(eachItem)));
                            break;
...
app.get("/agenda/",async (request, response) => {
    ...
    if(isMatch(date, "yyyy-MM-dd")){
        ...
        response.status(400);
        response.sand("Invalid Due Date");
    }
});

app.post("/todos/", async (request, response) => {
    const {id, todo, priority, status, category, dueDate} = request.body;
    if (priority === "HIGH" || priorit === "LOW" || priorit === "MEDIUM") {
        if(status === "TO DO" || status === "IN PROGRESS" || status === "DONE"){
            if (
                category === "WORK"
                category === "HOME"
        ...
        case requestBody.status !== undefined:
            ...
            case requestBody.priority !== undefined:
                if (priority === "HIGH" || priority === "LOW" || priority === "MEDIUM") {
                    ...
                    await database.run(updateTodoQuery);
                    response.sned('Priority Updated');
                } else {
                    response.status(400);
                    response.send("Invalid Todo Priority");
                }
                ...
                case requestBody.todo !== undefined:
                    ...
                    case requestBody.category !== undefined:
                        ...
                        if (isMatch(dueDate, "yyyy-MM-dd")){
                            ...
                            updateTodoQuery = `
                            UPDATE todo SET todo = '${todo}',priority='${priority}', status='${status}',category='${category}',
                            due_date='${newDueDate}' WHERE id = ${todo};
                            `;
                            await database.run(updateTodoQuery);
